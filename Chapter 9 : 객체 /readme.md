...
상속을 하는 이유
- 이미 정의된 프로퍼티와 메서드의 코드를 재사용할 수 있다
- 새로운 기능을 추가해서 확장된 객체를 만들 수 있다
- 중복 코드를 작성하지 않아도 되므로 유지 보수성이 높아진다.
...
객체의 \_\_proto\_\_ 프로퍼티는 그 객체에게 **상속을 해준 부모 객체**를 가리킨다. 따라서 객체는 \_\_proto\_\_ 프로퍼티가 가리키는 부모 객체의 프로퍼티를 사용할 수 있다.
```js
var objA = {
    name: "Tom",
...
이처럼 자신이 갖고 있지 않은 프로퍼티를 \_\_proto\_\_ 프로퍼티가 가리키는 객체를 차례대로 거슬러 올라가며 검색한다. 이를 **프로토타입 체인**이라고 한다. 
...
실제로 프로그래밍을 할 때는 \_\_proto\_\_ 프로퍼티 값을 직접 입력해서 상속하지는 않는다. 일반적으로 다음과 같은 방법으로 상속한다
...
instanceof 연산자는 지정한 객체의 프로토타입 체인에 지정한 생성자의 프로토타입 객체가 포함 되어있는지 판정한다. instanceof 연산자는 논리값을 반환하는 이항 연산자이다. instanceof 연산자는 인스턴스가 새당생성자로 생성되었는지 여부를 확인하는 것이 아닌, 생성자의 프로토타입 객체를 상속받았는 지이다.
...
person2.name = 'Huck'
person.sayHello(); // Hello! Huck
```

인수에 null을 넘기면 프로토타입없는 백지상태의 객체를 생성할 수 있다.
객체 리터럴로 생성한 빈 객체와 똑같은 객체를 생성하려면 인수로 Object.prototype을 넘기면 된다.
...
### 프로퍼티의 속성
프로퍼티는 다음과 같은 세 가지 내부 속성을 가지며 각 속성 값으 논리값이다
- 쓰기 가능
- 열거 가능
- 재정의 가능

객체에 새로운 프로퍼티를 추가하면 기본적으로 그 프로퍼티의 기본 속성이 쓰기,열거,재정의 가능으로 설정된다. 내장 생성자가 가지고 있는 프로토타입 객체의 프로퍼티 대부분의 내장 속성은 쓰기, 재정의 가능, 열거 불가능이다.    

### 프로퍼티가 있는지 확인하기
1. in 연산자 : 객체 안에 지명한 프로퍼티가 있는 지 검색. 검색 대상은 그 객체가 소유한 프로퍼티와 상속받은 프로퍼티 모두
2. hasOwnProperty : 그 객체가 소유한 프로퍼티일 경우만 true
3. propertyIsEnumerable: 지정한 프로퍼티가 그 객체가 소유한 프로퍼티이며 열거할 수 있을 때만 true

### 프로퍼티의 열거

1. for/in 문 : 객체와 객체의 프로토타입 체인에서 열거할 수 있는 프로퍼티 모두 꺼내어 반복
2. Object.keys : 지정한 객체가 소유한 프로퍼티 중에서 열거할 수 있는 프로퍼티 이름만 배열로 만들어서 반환
3. Object.getOwnPropertyNames: 인수로 지정한 객체가 소유한 프로퍼티 이름을 배열로 만들어서 반환. 열거 할 수 있는, 그리고 열거할 수 없는 프로퍼티 모두 배열로 만든다.

## 객체 잠그기

1. 확장 방지 : Object.preventExtensions : 프로퍼티 추가할 수 없다.
2. 밀봉 : Object.seal : 프로퍼티 추가 금지, 기존 프로퍼티 재정의 금지. 읽기와 쓰기만 가능
3. 동결 : Object.freeze : 프로퍼티 추가 금지, 재정의 금지, 데이터 프로퍼티 쓰기 금지. 읽기만 가능
